...
-- ATTENTION: THEY MAY BE OUTDATED!
...
-- CREATE VIEW potencial_a_cobrar_x_barrio AS
SELECT 
  tparametro.semana, 
  beneficiaria.barrio_id, 
  barrio.nombre AS barrio_nombre, 
  sum(credito.deuda_total / credito.cuotas) AS recaudacion_potencial 
FROM (
  SELECT DISTINCT 
    yearweek(parametro.fecha, 0) AS semana 
  FROM 
    parametro
  ) AS tparametro, 
  credito 
  ...
  INNER JOIN barrio ON barrio.id = beneficiaria.barrio_id 
WHERE 
  yearweek(adddate(credito.fecha_entrega, 14), 0) <= tparametro.semana 
  AND (yearweek(credito.fecha_finalizacion, 0) >= tparametro.semana 
       OR credito.fecha_finalizacion IS NULL) 
GROUP BY 
  tparametro.semana, 
  beneficiaria.barrio_id;
...
SELECT 
  concat(mid(potencial_a_cobrar_x_barrio.semana, 1, 4), '.', mid(potencial_a_cobrar_x_barrio.semana, 5, 2)) AS semana, 
  potencial_a_cobrar_x_barrio.barrio_nombre AS barrio, 
  ifnull(rec_real.recaudacion, %s) AS recaudacion, 
  potencial_a_cobrar_x_barrio.recaudacion_potencial, 
  ifnull(rec_real.recaudacion, %s) / potencial_a_cobrar_x_barrio.recaudacion_potencial AS porcentaje 
FROM 
  potencial_a_cobrar_x_barrio 
  LEFT OUTER JOIN (
    SELECT 
      yearweek(pago.fecha) AS semana, 
      beneficiaria.barrio_id AS barrio_id, 
      barrio.nombre AS barrio_nombre, 
      sum(pago.monto) AS recaudacion 
    FROM 
      pago 
      INNER JOIN credito ON credito.id = pago.credito_id 
      INNER JOIN beneficiaria ON beneficiaria.id = credito.beneficiaria_id 
      INNER JOIN barrio ON barrio.id = beneficiaria.barrio_id 
    WHERE 
      pago.fecha >= (SELECT min(parametro.fecha) AS min_1 FROM parametro) 
    ...
    GROUP BY 
      yearweek(pago.fecha), 
      beneficiaria.barrio_id
  ) AS rec_real ON potencial_a_cobrar_x_barrio.semana = rec_real.semana AND potencial_a_cobrar_x_barrio.barrio_id = rec_real.barrio_id;
